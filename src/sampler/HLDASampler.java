...
public class HLDASampler extends AbstractSampler{
     ...
     * path assignments. 
     * 
     * Here, we need to consider all possible paths
     * - existing paths, each of which is represented by a leaf node
     * - novel paths, each of which is represented by an internal node
     * 
     * For each path c_d, we need to compute two things
     * - The prior on c_d implied by the nested CRP: P(c_d | c_{-d})
     * - The likelihood of the data given a particular choice of c_d: P(w_m | c, w_{-m}, z)
     * 
...
//        logln("---> samplerIndex = " + sampledIndex + ". node = " + node.toString());
     ...
     * possible paths corresponds to the union of 
     * - the set of existing paths through the tree, each represented by a leaf
     * - the set of possible novel paths, each represented by an internal node
     * 
     ...
     * likelihood for new path at each level. Here each level share 1 value since
     * all multinomials are assumed to be generated from a shared symmetric
     * Dirichlet prior
     ...
     * @param curNode The node (which contains a multinomial over the vocabulary)
     ...
     * @param parentLogProb The log probability from the parent node that is passed to
     * the child node
     */
     ...
     * thrown. The number of customers at each node on the path will be decremented.
