...
//    protected HashMap<DNCRPNode, Double>[] jointPathLogProbs;
    ...
    /** Initialize a tree at 2nd time epoch onwards. This is done by copying 
     * all nodes from the previous tree which have positive number of pseudo
     * documents
...
//                    );
     ...
     * @param lookAhead Whether the
     * assignments in the next few epochs should be
     * considered
     */
     ...
     * will return a hash table containing
     * - Key: The set of nodes in the tree at the given time epoch. Each node
     * represents a possible path to assign a document.
     * - Value: The log transition probability of assigning a document to each
     * possible path
     * 
     * @param Time epoch
     */
...
//                    logln("non-leaf: " + curNode + "\t" + pathOldLogProb + ". " + pathNewLogProb + ". " + (pathNewLogProb - pathOldLogProb));
    ...
    /** Compute the possible joint log probability of all nodes in a tree if
     * a customer is added to any node in a tree delta epochs before
     * 
    ...
    private HashMap<DNCRPNode, HashMap<Integer, Double>> computePossibleJointPathLogProbability(DNCRPNode root, int delta){
        HashMap<DNCRPNode, HashMap<Integer, Double>> nodeLogProbs 
                = new HashMap<DNCRPNode, HashMap<Integer, Double>>();
        Stack<DNCRPNode> stack = new Stack<DNCRPNode>();
        stack.add(root);
        while(!stack.isEmpty()){
            DNCRPNode node = stack.pop();
    ...
    /** Compute the joint assignment log probability of an internal node if
     * a customer is added to a child of the delta-previous node of this node.
     * 
     ...
     * 
     * @param nodeLogProbs Hash table to store the result. Set of keys are all internal nodes
    ...
    /** Compute the joint assignment log probability of a given node. This can 
     * be viewed as a Chinese restaurant, but at the beginning there are 
     * non-empty tables, occupied by pseudo counts.
     * 
     ...
     * 
     * If there is no customers currently being assigned to this node, sample
     * a new topic from Gaussian(pre-node-mean, sigma^2 * I)
     * 
     ...
     * the log likelihood of each sample and averaging over the Q log likelihoods
     * 
