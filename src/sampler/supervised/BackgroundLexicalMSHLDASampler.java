 ...
 * @author vietan
 */
public class BackgroundLexicalMSHLDASampler extends AbstractSampler {
    ...
    public double[] getRegressionValues() {
        ...
        for (int d = 0; d < D; d++) {
            regValues[d] = (docTopicWeights[d] / docTokenCounts[d]) + docLexicalWeights[d];
        }
        return regValues;
    }
    ...
    private void sampleTableForSentence(int d, int s, boolean remove, boolean add,
        ...
        for (STable table : localRestaurants[d].getTables()) {
            ...
            if (observed) {
                ...
                double mean = docLexicalWeights[d] + ((docTopicWeights[d] + addTopicWeight) / docTokenCounts[d]);
    ...
    private void sampleLevelForToken(int d, int s, int n, boolean remove, boolean add, boolean observed) {
        ...
        if (observed) {
            ...
            if (z[d][s][n] == 0) {
                docLexicalWeights[d] -= lexicalWeights[words[d][s][n]];
            }
        }
        ...
        for (int l = 0; l < L; l++) {
            ...
            double resLlh = 0.0;
            if (observed) {
                double mean = docLexicalWeights[d]
                        + ((docTopicWeights[d] + curPath[l].getRegressionParameter()) / docTokenCounts[d]);
                resLlh = StatisticsUtils.logNormalProbability(responses[d], mean, sqrtRho);
            }
...
//        HashMap<SNode, Integer> nodeIndices = new HashMap<>();
...
//    }
    ...
    private void validateAssignments(String msg) {
        ...
        for (int d = 0; d < D; d++) {
            ...
            if (Math.abs(topicWeight - docTopicWeights[d]) > 0.001) {
                throw new RuntimeException(msg + ". Topic weights of document " + d
                        + " mismatch. " + topicWeight + " vs. " + docTopicWeights[d]);
            }

            double lexiconWeight = 0.0;
            for (int s = 0; s < words[d].length; s++) {
                for (int n = 0; n < words[d][s].length; n++) {
                    if (z[d][s][n] == 0) {
                        lexiconWeight += lexicalWeights[words[d][s][n]];
                    }
                }
            }
            if (Math.abs(lexiconWeight - docLexicalWeights[d]) > 0.001) {
                throw new RuntimeException(msg + ". Lexicon weight of document " + d
                        + " mismatch. " + lexiconWeight + " vs. " + docLexicalWeights[d]);
            }
        }
    }
...
//            double[] predResponses = regressNewDocumentsDebug(
//                    reportFolderpath + filename, 
//                    newWords
//                    );
