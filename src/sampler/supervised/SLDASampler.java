 ...
 * @author vietan
 */
public class SLDASampler extends AbstractSampler{
     ...
     * @param n Token index
     * @param remove Whether this token should be removed from current state
     * @param add Whether this token should be added 
    ...
    private void sampleZ(int d, int n, boolean remove, boolean add, boolean observe){
        ...
        for(int k=0; k<K; k++){
            ...
            if(observe){
                ...
                logprobs[k] += StatisticsUtils.logNormalProbability(responses[d], mean, Math.sqrt(hyperparams.get(RHO)));
            }
        }
...
//    public double[] regressNewDocumentsFinal(
//            int[][] newWords, 
//            double[] newResponses,
//            String partialResultFolder
//            ) throws Exception{
//        String finalModelFilepath = this.folder + this.getSamplerFolder() + "final.zip";
//        double[] predResponses = regressNewDocuments(finalModelFilepath, newWords, partialResultFolder + "final.txt");
//        
//        if(verbose){ // for debugging only
//            logln("state file: " + finalModelFilepath);
//            RegressionEvaluation eval = new RegressionEvaluation(
...
//    }
    ...
    public double[] regressNewDocuments(
        ...
        IOUtils.createFolder(partialResultFolder);
        for(int i=0; i<filenames.length; i++){
            String filename = filenames[i];
            if(!filename.contains("zip"))
                continue;
            ...
            double[] predResponses = regressNewDocuments(
                    reportFolderpath + filename, 
                    newWords,
                    partialResultFolder + IOUtils.removeExtension(filename) + ".txt"
                    );
