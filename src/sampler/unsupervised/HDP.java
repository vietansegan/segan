...
public class HDP extends AbstractSampler {
    ...
    public void configure(String folder, int V, int K,
    ...
    private void initializePresetAssignments() {
        ...
        for (int kk = 0; kk < K; kk++) {
            ...
            topicWords.createNewComponent(kk, topicWord);
        }
        ...
        for (int dd = 0; dd < D; dd++) {
            for (int nn = 0; nn < words[dd].length; nn++) {
                ...
                topicWords.getComponent(z[dd][nn]).increment(words[dd][nn]);
            }
        }
    ...
    private void sampleZs(boolean removeFromModel, boolean addToModel,
        ...
        for (int ii = 0; ii < D; ii++) {
            ...
            for (int nn = 0; nn < words[dd].length; nn++) {
                ...
                if (removeFromModel) {
                    this.topicWords.getComponent(curZ).decrement(words[dd][nn]);
                    if (this.topicWords.getComponent(curZ).isEmpty()) {
                ...
                for (int k : topicWords.getIndices()) {
                    ...
                    double topicWordProb =
                            (topicWords.getComponent(k).getCount(words[dd][nn])
                            + hyperparams.get(BETA))
                            / (topicWords.getComponent(k).getCountSum() + totalBeta);
                ...
                if (newZ == NEW_COMPONENT_INDEX) {
                    ...
                    topicWords.createNewComponent(newZ, topicWord);
                ...
                if (addToModel) {
                    this.topicWords.getComponent(newZ).increment(words[dd][nn]);
                }
    ...
    public void validate(String msg) {
        ...
        for (int k : topicWords.getIndices()) {
            topicWords.getComponent(k).validate(msg);
        }
    }
    ...
    public void outputState(String filepath) {
        ...
        try {
            ...
            for (int k : topicWords.getSortedIndices()) {
                ...
                modelStr.append(DirMult.output(topicWords.getComponent(k))).append("\n");
            }
    ...
    protected void inputModel(String zipFilepath) {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                this.topicWords.createNewComponent(k, topicWord);
            }
    ...
    public void outputTopicTopWords(File file, int numTopWords) throws Exception {
        ...
        for (int k : topicWords.getIndices()) {
            sortedTopics.add(new RankingItem<Integer>(k, topicWords.getComponent(k).getCountSum()));
        }
        ...
        for (int ii = 0; ii < sortedTopics.size(); ii++) {
            ...
            double[] distrs = topicWords.getComponent(k).getDistribution();
    ...
    class Topics {
        ...
        private HashMap<Integer, DirMult> actives;
        private SortedSet<Integer> inactives;

        public Topics() {
            this.actives = new HashMap<Integer, DirMult>();
        ...
        public DirMult getComponent(int idx) {
        ...
        public void createNewComponent(int idx, DirMult c) {
            if (isActive(idx)) {
